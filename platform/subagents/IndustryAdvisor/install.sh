...
SKILL_NAME="sf-industry-advisor"
REPO_URL="https://github.com/sfdc-brendan/Unofficial-Skills.git"
REPO_SUBPATH="platform/subagents/IndustryAdvisor"
...
banner() {
  echo ""
  echo -e "${CYAN}╔════════════════════════════════════════════════════════╗${NC}"
  echo -e "${CYAN}║  Salesforce Industry Advisor Subagent                 ║${NC}"
  echo -e "${CYAN}║  Vertical expertise & regulatory context              ║${NC}"
  echo -e "${CYAN}╚════════════════════════════════════════════════════════╝${NC}"
  echo ""
}
...
install_to() {
  ...
  if [ -f "$script_dir/SKILL.md" ]; then
    ...
    cp "$script_dir/references/industry-patterns.md" "$target_dir/references/industry-patterns.md"
  else
    ...
    cp "$tmp_clone/$REPO_SUBPATH/references/industry-patterns.md" "$target_dir/references/industry-patterns.md"
    rm -rf "$tmp_clone"
  fi
...
if [ "${1:-}" = "--cursor" ]; then
  ...
  echo "  curl -fsSL https://raw.githubusercontent.com/sfdc-brendan/Unofficial-Skills/main/platform/subagents/IndustryAdvisor/install.sh | bash -s -- --both"
  echo ""
  exit 0
fi

echo ""
echo -e "${GREEN}Done!${NC} The Industry Advisor subagent will auto-activate when you:"
echo "  • Work on industry-specific Salesforce implementations"
echo "  • Need regulatory compliance guidance (HIPAA, SOX, FedRAMP, etc.)"
echo "  • Map industry terminology to Salesforce objects"
echo "  • Connect platform capabilities to industry KPIs"
echo ""
