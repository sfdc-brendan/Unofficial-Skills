...
SKILL_NAME="sf-release-manager"
REPO_URL="https://github.com/sfdc-brendan/Unofficial-Skills.git"
REPO_SUBPATH="platform/subagents/ReleaseManager"
...
banner() {
  echo ""
  echo -e "${CYAN}╔════════════════════════════════════════════════════════╗${NC}"
  echo -e "${CYAN}║  Salesforce Release Manager Subagent                  ║${NC}"
  echo -e "${CYAN}║  CI/CD, environments & change management             ║${NC}"
  echo -e "${CYAN}╚════════════════════════════════════════════════════════╝${NC}"
  echo ""
}
...
install_to() {
  ...
  if [ -f "$script_dir/SKILL.md" ]; then
    ...
    cp "$script_dir/references/release-patterns.md" "$target_dir/references/release-patterns.md"
  else
    ...
    cp "$tmp_clone/$REPO_SUBPATH/references/release-patterns.md" "$target_dir/references/release-patterns.md"
    rm -rf "$tmp_clone"
  fi
...
if [ "${1:-}" = "--cursor" ]; then
  ...
  echo "  curl -fsSL https://raw.githubusercontent.com/sfdc-brendan/Unofficial-Skills/main/platform/subagents/ReleaseManager/install.sh | bash -s -- --both"
  echo ""
  exit 0
fi

echo ""
echo -e "${GREEN}Done!${NC} The Release Manager subagent will auto-activate when you:"
echo "  • Plan releases or set release cadence"
echo "  • Design CI/CD pipelines or environment strategy"
echo "  • Create deployment checklists or rollback procedures"
echo "  • Write change management plans or release notes"
echo "  • Manage sandboxes or compare org metadata"
echo ""
