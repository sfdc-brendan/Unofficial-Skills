...
SKILL_NAME="sf-customer-success"
REPO_URL="https://github.com/sfdc-brendan/Unofficial-Skills.git"
REPO_SUBPATH="platform/subagents/CustomerSuccess"
...
banner() {
  echo ""
  echo -e "${CYAN}╔════════════════════════════════════════════════════════╗${NC}"
  echo -e "${CYAN}║  Salesforce Customer Success Subagent                 ║${NC}"
  echo -e "${CYAN}║  Adoption, health & expansion                        ║${NC}"
  echo -e "${CYAN}╚════════════════════════════════════════════════════════╝${NC}"
  echo ""
}
...
install_to() {
  ...
  if [ -f "$script_dir/SKILL.md" ]; then
    ...
    cp "$script_dir/references/success-frameworks.md" "$target_dir/references/success-frameworks.md"
  else
    ...
    cp "$tmp_clone/$REPO_SUBPATH/references/success-frameworks.md" "$target_dir/references/success-frameworks.md"
    rm -rf "$tmp_clone"
  fi
...
if [ "${1:-}" = "--cursor" ]; then
  ...
  echo "  curl -fsSL https://raw.githubusercontent.com/sfdc-brendan/Unofficial-Skills/main/platform/subagents/CustomerSuccess/install.sh | bash -s -- --both"
  echo ""
  exit 0
fi

echo ""
echo -e "${GREEN}Done!${NC} The Customer Success subagent will auto-activate when you:"
echo "  • Create success plans or adoption scorecards"
echo "  • Build health check reports or health scores"
echo "  • Map expansion opportunities or prepare QBR decks"
echo "  • Design risk mitigation plans or renewal strategies"
echo "  • Analyze feature adoption or synthesize customer feedback"
echo ""
